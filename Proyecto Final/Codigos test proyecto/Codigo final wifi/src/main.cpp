...
//█████████████████████████████████████████████████████████████████████████████████
//Cargamos las paginas web a traves de una cadena de char constantes 
//Index:
...
// Set by timer:
const char settimer_html[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html>
<head>
  ...
  <script>
    ...
    function updateSliderMINUTE(element) {
        ...
        console.log(MVal);
        document.getElementById("MSV").innerHTML = MVal;
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "/slider?minute="+MVal, true);
        xhr.send();
      }
    </script>
    <p><button class="save1" onclick=location.href="/S1" >Save in slot 1</button></p> 
    <p><button class="save2" onclick=location.href="/S2" >Save in slot 2</button></p> 
    <p><button class="save3" onclick=location.href="/S3" >Save in slot 3</button></p> 
...
void setup(){
  ...
  { 
    request->send_P(500, "text/html", index_html, processor);
  });
  server.begin();
}
