...
int8_t ArrayToChar(uint8_t function, char buffer[20]) //// ya arregle lo de colver
{
  ...
  case 1:
    ...
    if (resto < 8)
      Aux1 = Aux1 - resto; // utiliza el resto para redondear abajo (Esto se da pq en el propio diseño del sistema decidimos guardar todas las horas en un char)
    else
      Aux1 = Aux1 + 15 - resto; // utiliza el resto para redondear arriba
...
void encoder_value(uint8_t value, uint8_t funcion)
{
  ...
  if (funcion == 3)
    encoder0Pos -= value;
  if (funcion == 4)
    encoder0Pos = ReturnToCero(encoder0Pos, value);
}
