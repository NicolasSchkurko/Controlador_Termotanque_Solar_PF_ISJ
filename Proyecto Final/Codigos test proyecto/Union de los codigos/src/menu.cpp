...
typedef enum
{
  ...
  llenado_manual,
  calefaccion_manual,
  funcion_menu_de_auto_por_hora_display,
  llenado_auto,
  calefaccion_auto,
  funcion_de_menu_modificar_hora_display_rtc,
...
void standby(bool Display_farenheit, uint8_t nivel_display, int8_t temperatura_display, uint8_t hora_display, uint8_t minutos_display, uint8_t posicion_encoder)
{
  if (Display_farenheit == false)
    sprintf(imprimir_lcd, "T:%d%cC", temperatura_display, (char)223);
  if (Display_farenheit == true)
    sprintf(imprimir_lcd, "T:%d%cF", ((9 * temperatura_display) / 5) + 32, (char)223);
  PrintLCD(imprimir_lcd, 0, 0);
  sprintf(imprimir_lcd, "N:%d%c", nivel_display, '%');
  PrintLCD(imprimir_lcd, 12, 0);
  Printhora(imprimir_lcd, hora_display, minutos_display);
...
void menu_basico(uint8_t posicion_encoder)
{
  ...
  case 1: //¿Por que hago tiempo actual -=100? en el lcd se ve mejor cuando el tiempo de "apagado" es menor al de encendio
    ...
    PrintLCD(imprimir_lcd, 0, 0);
    sprintf(imprimir_lcd, "%s", Menuprincipal[ReturnToCero(Ypos, maxY_menu1)]);
    PrintLCD(imprimir_lcd, 1, 0);
    sprintf(imprimir_lcd, "%s", Menuprincipal[ReturnToCero(Ypos + 1, maxY_menu1)]);
    PrintLCD(imprimir_lcd, 1, 1);
    sprintf(imprimir_lcd, "%s", Menuprincipal[ReturnToCero(Ypos + 2, maxY_menu1)]);
    PrintLCD(imprimir_lcd, 1, 2);
    sprintf(imprimir_lcd, "%s", Menuprincipal[ReturnToCero(Ypos + 3, maxY_menu1)]);
    ...
    if (PressedButton(1))
      encoder_value(2, 2); // suma 1 a Ypos
    if (PressedButton(2))
      encoder_value(2, 3); // resta 1 a Ypos
    if (PressedButton(3))
    {
...
void menu_avanzado(uint8_t posicion_encoder)
{
  ...
  case 1:
    sprintf(imprimir_lcd, "%c", char(62));
    PrintLCD(imprimir_lcd, 0, 0);
    sprintf(imprimir_lcd, "%s", menuavanzado[ReturnToCero(Ypos, maxY_menu2)]);
    PrintLCD(imprimir_lcd, 1, 0);
    sprintf(imprimir_lcd, "%s", menuavanzado[ReturnToCero(Ypos + 1, maxY_menu2)]);
    PrintLCD(imprimir_lcd, 1, 1);
    sprintf(imprimir_lcd, "%s", menuavanzado[ReturnToCero(Ypos + 2, maxY_menu2)]);
    PrintLCD(imprimir_lcd, 1, 2);
    sprintf(imprimir_lcd, "%s", menuavanzado[ReturnToCero(Ypos + 3, maxY_menu2)]);
    PrintLCD(imprimir_lcd, 1, 3);
